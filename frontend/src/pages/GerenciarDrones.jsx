...
const GerenciarDrones = () => {
  ...
  const unassign = async (droneId, deliveryId) => {
    if (!window.confirm('Remover esta entrega do drone e devolvê-la para aguardando confirmação?')) return;
    setError('');
    try {
      await api.delete(`/drones/${droneId}/entregas/${deliveryId}`);
      await loadData();
    } catch (err) {
          ...
          return <article key={drone.id} style={{ padding: '22px', borderRadius: '18px', background: 'white', boxShadow: '0 8px 24px rgba(16,35,61,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
              ...
              <label style={{ color: '#58708d' }}>Status
                <select value={drone.status || 'DISPONIVEL'} onChange={(event) => updateStatus(drone.id, event.target.value)} style={{ display: 'block', marginTop: '5px', padding: '9px', borderRadius: '8px', border: '1px solid #d6deea', background: 'white' }}>
            ...
            <div style={{ display: 'grid', gap: '9px' }}>
              {assigned.map((delivery) => <div key={delivery.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', padding: '12px', borderRadius: '10px', background: '#f7f9fc' }}>
                ...
                <button onClick={() => unassign(drone.id, delivery.id)} style={{ padding: '8px 11px', border: 0, borderRadius: '8px', background: '#fee2e2', color: '#c53030', fontWeight: 700, cursor: 'pointer' }}>Remover</button>
              </div>)}
